...
지정 디렉토리의 모든 JPG/PNG 파일을 비율 기반으로 압축한다.

- **1차 압축**: 원본 대비 -50% 목표
- **2차 압축**: 1차 결과가 500kb 이상이면 -60% 목표로 재압축
- **JPEG**: mozjpeg 인코더, quality 80부터 10까지 10씩 줄이며 목표 달성 시 저장
- **PNG**: palette 양자화, 256 → 128 → 64 → 32 → 16 colours 단계적 감소
- 결과: `photo.jpg` → `photo_mini_320kb.jpg` (파일명에 압축 후 용량 명시, 원본 삭제)
